...
| `list_pages` | `prefix: str = "", contains: str = ""` | `GET /.fs` then filter in Python (filter happens *before* hydration so the prefix reduces the per-page round-trip count) | `list[{name, etag, size_bytes, last_modified_ms, created_ms}]` (T28 widened from the v1.1 minimal subset; ``etag`` is `None` for every row on this SB build because the list payload omits the field — an operator who needs ``if_match`` round-trips opts in to per-page hydration via `MCP_SILVERBULLET_LIST_PAGES_HYDRATE_ETAGS`) | `prefix=` keeps v1's `startswith` semantics (wire surface unchanged for v1 / v1.1 / v1.2 / v1.3 callers); `contains=` (T37) does substring matching against the page name and composes as AND with `prefix` when both are set; either empty is a no-op for that criterion; both empty returns the full listing. The filter only ever matches against page *names* — body-content search lives behind the journal gate (T38; see the README's Discovery tools (journal-gated) section) |
...
| `412 Precondition Failed` | `write_page` / `delete_page` / `append_to_page` / `patch_page_lines` / `patch_page_replace` / `move_page` (on the delete step) / `check_task` (T30, on the write step) return `ToolError("precondition failed; check if_match/if_none_match")`. `move_page`'s destination-collision 412 gets the special-case wording (see the tool row above). A 412 on a `list_pages` per-page hydration GET (T28) leaves that row's `etag` as `null` (proxy / SB misconfig, not an agent error). `diff_pages` (T27) — and `check_task`'s read step (T30) — have no precondition surface; a 412 on a read is highly unusual (a GET normally doesn't carry `If-Match`) but surfaces as the same wording if a proxy / SB misconfig triggers one. `check_task` *also* validates the precondition against the read's etag on the dry-run path so a stale-etag `dry_run=True` raises the same wording without issuing a write. `create_page` (T32) intercepts `PreconditionFailed` *before* `_translate_sb_errors` and re-raises as `ToolError("page already exists: {name}; use write_page to overwrite")` (a clean next-tool hint rather than the generic 412 wording; on SBs that don't honor `If-Match`, `create_page` silently overwrites — a `T32a` follow-up could close the gap). **T31b caveat**: on SBs that don't honor `If-Match` (T31's negative finding on this dev box), a 412 is not enforced — the write returns 200 on a stale etag. The bridge's post-write verification helper (`_verify_concurrency_token` in `server.py`) re-reads after the PUT and compares the verification-GET etag against the PUT-response etag (the bridge's view of "what we just wrote"); a mismatch means a concurrent writer touched the page between the bridge's PUT and the re-read, and the helper raises `ToolError("concurrent edit detected on {name}: the page changed since we wrote at {expected_etag}; current etag is {current_etag} — re-issue the write with if_match={current_etag}")` (T46 wording — see T45 paragraph below for the v1.5 → v1.6 wording evolution). On SBs that *do* honor `If-Match`, the 412 path above still wins (cheaper, fires before the helper). **T46 fix**: the helper's comparison reference changed from the caller's pre-write `if_match` (T31b's original semantic — caught read-vs-write races via the caller's stale etag) to the PUT-response etag (the bridge's view of "what we just wrote" — catches races between the bridge's PUT and the verification GET). The pre-T46 helper raised a spurious "concurrent edit detected" on every read-modify-write that grew the page (the synthesized etag is `str(size_bytes)` per T44, and the post-write size differs from the pre-write size on every append / prepend / patch / move). Live reproduction on this dev box: 76 spurious errors in 6 hours on `Trading Book/Logs/2026-W36.md`, every one with `current_etag - expected_etag` exactly equal to the appended content length. The T46 fix is structural — the read-modify-write tools (the dominant agent workflow) succeed on a non-race write; the verification helper's detection semantic narrows to the narrow PUT-to-GET window in which a concurrent writer can land a PUT on the same page. A caller-supplied stale `if_match` (the agent manages its own etag round-trip and the page drifted in the gap) no longer fires the helper — that defense moved to the agent side (re-read on the agent's own retry loop, or use the bridge's read-modify-write tools which do their own internal re-read and pass the verification check). **T45 wording change**: the T31b helper embeds the post-write etag directly in the error message (as `current etag is "<etag>" — re-issue the write with if_match="<etag>"`) so the agent has the literal `if_match=` value for the next call without an extra read round trip; the bridge just did the read for them. The pre-T45 / pre-v1.6 wording quoted above is the T31b baseline; the v1.6 wording is byte-additive over it (an agent that pattern-matches on the bare `concurrent edit detected` prefix still matches). T46 re-anchored the `expected_etag` placeholder from "the page changed since you read it at" to "the page changed since we wrote at" — the comparison now references the bridge's PUT, not the agent's read, so the framing shifts accordingly; the `expected_etag` placeholder carries the PUT-response etag (the bridge's view of what it wrote) rather than the caller's pre-write etag. **T44 caveat**: the synthesized-etag primitive (the value the helper compares against on SBs that strip `ETag`) is now `"{size_bytes}"` alone — pre-T44 it was `"{last_modified_ms}-{size_bytes}"`. The bridge stamps `X-Last-Modified` with `now_ms` on every PUT request (`_WRITE_HEADERS`), which made the dashed form drift on every write even when the body was unchanged; T44 dropped the mtime component so the synthesized etag is stable across re-reads of the same body. The wire-shape change is backwards-incompatible for v1.3 / v1.4 callers holding a synthesized etag across calls — re-read once after the fix lands to pick up the new canonical form. **T42 hint**: after `_CONTENTION_THRESHOLD` (3) 412s on the same page within `_CONTENTION_WINDOW_SECONDS` (60), the *next* 412 `ToolError` on that page appends ` [concurrent_edit_hint: true]` to the standard message — an agent in a contention loop gets a clear back-off signal. The hint is purely additive: a 412 outside the window uses the standard wording (with the v1.6 T45 `; read_page("<name>") for the current etag and re-issue` suffix but without the T42 marker suffix), an agent that doesn't check for the marker pattern-matches on `precondition failed` as before. See `server._contention_hint` for the sliding-window counter (per-process, per-name; not persistent across restarts). **T45 retry guidance**: the v1.6 surface widens both 412 messages with actionable retry hints so an agent that followed the concurrency protocol correctly doesn't have to guess what the next call should look like. The standard 412 (`_translate_sb_errors`'s `PreconditionFailed` clause) gains a `; read_page("<name>") for the current etag and re-issue` suffix — the resolved page name (T39's design call) appears as a literal copy-paste-able token, so the agent sees exactly which read to call. The standard path can't embed the current etag directly (SB's 412 response body is empty on this build), so the wording points at the read instead. `list_pages`'s 412 has no per-page name — the empty-name guard skips the pointer and the bare prefix is enough. The silent-overwrite 412 (`_verify_concurrency_token`'s helper) widens `_CONCURRENT_EDIT_MSG` from the v1.5 single-placeholder form (`{expected_etag}`) to a three-placeholder form (`{name}`, `{expected_etag}`, `{current_etag}`): the bridge has the post-write etag in hand from the verification re-read, and the wording embeds it directly as `current etag is "<etag>" — re-issue the write with if_match="<etag>"`, so the agent has the literal `if_match=` value for the next call without an extra read round trip. Both surfaces are byte-additive over the v1.5 wording — an agent that pattern-matches on the bare `precondition failed` (standard path) or `concurrent edit detected` (silent-overwrite path) prefix still matches; only agents that pinned the byte-for-byte full message (a small set of v1.5 / earlier tests) need to update. The T42 `[concurrent_edit_hint: true]` marker rides as the trailing suffix on the standard 412 message *after* the T45 suffix, so the full contention-window message becomes `...; read_page("Foo.md") for the current etag and re-issue [concurrent_edit_hint: true]`. **T46 fix**: the silent-overwrite helper's comparison reference changed from the caller's pre-write `if_match` to the PUT-response etag, narrowing the detection semantic to *between the bridge's PUT and the verification GET* (the window in which a concurrent writer can land a PUT on the same page). This is a structural fix — the helper no longer raises a spurious "concurrent edit detected" on every read-modify-write that grows the page (the dominant agent workflow — Grok's `append_to_page` calls on the W36 log page, etc.). The wording's `expected_etag` placeholder carries the PUT-response etag (the bridge's view of "what we wrote"); `current_etag` carries the verification-GET etag (the next-call etag). The framing shifts from "since you read it at" to "since we wrote at" — the comparison now references the bridge's PUT, not the agent's pre-write read. **T47 auto-retry**: the six read-modify-write tools (`append_to_page`, `prepend_to_page`, `patch_page_lines`, `patch_page_replace`, `check_task`, `move_page`) gain a `max_retries: int = 3` parameter that auto-retries the read-modify-write block when the T31b / T46 helper fires `concurrent edit detected`. On each retry the bridge re-reads the body, re-derives the operation against the page's *current* state, and re-PUTs. Default-on (`max_retries=3`); pass `max_retries=0` to opt out and see the raw 412 (matches the pre-T47 behavior). Genuine semantic errors (`find not found in body`, `page not found`, body-size errors) surface to the agent unchanged — the bridge retries only on the post-write-verification race, not on anchor-mismatch or 404. The standard-412 path (SB honored `If-Match` and returned 412) is *not* auto-retried: an agent that passed an explicit stale `if_match` should see the 412 — retrying would mask the precondition failure. |
...
| `5xx` | `ToolError("silverbullet error: <status>")` — including for `page_exists`, where 5xx deliberately returns an error rather than `False` so the caller can distinguish "no, proceed" from "SB is broken, don't make decisions". A 5xx on a `list_pages` per-page hydration GET (T28) leaves that row's `etag` as `null` (transient SB hiccup, not an agent error). **T43 cf_hint**: when the 5xx body is Cloudflare-shaped (carries `cloudflare_error` / `error_category` / `ray_id`), the bridge parses the JSON envelope and appends a `[cf_hint: {...}]` marker to the standard `ToolError` message. The marker carries three fields useful for an agent's retry decision: `retry_after` (seconds, may be `None` if upstream omitted it), `error_code` (the numeric CF error code, e.g. `502`), and `title` (the human-readable summary, e.g. `"Error 502: Bad gateway"`). The marker rides on the message-text channel (not a native envelope field) because the MCP SDK renders `ToolError` to the wire as plain `TextContent(text=str(exc))` — same shape as T42's `concurrent_edit_hint`. An agent that knows the marker can `json.loads` it directly to act on `retry_after`; an agent that doesn't see no behavior change (the marker is conditional — non-CF 5xx bodies leave the standard wording unchanged). Other CF fields (`ray_id`, `zone`, `instance`, `error_name`, `timestamp`, `what_you_should_do`) are deliberately dropped: they're useful for debugging the CF/proxy layer, not for the agent's decision-making. The bridge never re-tries on `cf_hint` itself — the agent decides whether to retry based on `retry_after`. |
...
- **Body-substring search without the journal gate.** SilverBullet
  exposes no HTTP search endpoint (the `/.fs` API is read-by-name and
  list-by-directory, with no body-content index); the bridge has no
  way to substring-search page bodies without filesystem access to
  the SB space directory. The journal gate (`MCP_SILVERBULLET_JOURNAL_TOOLS=1`
  + `MCP_SILVERBULLET_SPACE_PATH`) is the only path. Operators
  without that gate can still narrow by name via `list_pages`
  (`prefix=` / `contains=`, T37); operators who want body search
  enable the gate and use `pages_touching_topic` / `search_pages`.
  T38 surfaces the gate more loudly in the README and `list_pages`'s
  description so an operator who hits a "no body search" dead end
  has an obvious next step rather than relitigating the scope.
